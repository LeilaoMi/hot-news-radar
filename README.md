...
- [ ] **P2-② 热度趋势图表**：`build_trends.py` 目前是统计雏形，补充时序图表可视化
- [ ] **P1-2 拆分 `trendradar/report/html.py`**（3200+ 行单函数）：已备施工图，
      需先做快照特征化测试锁定输出再动手
- [ ] **P1-4 长尾**：hot path 的 `except Exception` 逐步收敛为具体异常（渐进，不专项批改）
- [ ] **MCP 订阅管理工具**：通过 MCP Server 管理关键词订阅（`gen_feed.py` 已有基础）
- [ ] **观察期**：语义去重 / AI 日报上线后的前几个 Actions 周期，
      关注日志确认 AI 判定分组准确率与日报生成时机
- [ ] 语义去重试用稳定后评估默认开启
