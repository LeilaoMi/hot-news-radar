...
def send_to_feishu(
    ...
    *,
    ...
    log.info(f"{log_prefix}消息分为 {len(batches)} 批次发送 [{report_type}]")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("StatusCode") == 0 or result.get("code") == 0:
                    log.info(f"{log_prefix}第 {i}/{len(batches)} 批次发送成功 [{report_type}]")
                    # 批次间间隔
                    if i < len(batches):
            ...
            log.info(f"{log_prefix}第 {i}/{len(batches)} 批次发送出错 [{report_type}]：{e}")
            return False

    log.info(f"{log_prefix}所有 {len(batches)} 批次发送完成 [{report_type}]")
...
def send_to_wework(
    ...
    *,
    ...
    if is_text_mode:
        log.info(f"{log_prefix}使用 text 格式（个人微信模式）[{report_type}]")
    else:
        log.info(f"{log_prefix}使用 markdown 格式（群机器人模式）[{report_type}]")
    ...
    *,
    ...
    try:
        if not html_file_path or not Path(html_file_path).exists():
            log.info(f"错误：HTML文件不存在或未提供: {html_file_path}")
            return False

        log.info(f"使用HTML文件: {html_file_path}")
        ...
        if custom_smtp_server and custom_smtp_port:
            ...
            log.info(f"未识别的邮箱服务商: {domain}，使用通用 SMTP 配置")
...
请使用支持HTML的邮件客户端查看完整报告内容。
        ...
        log.info(f"正在发送邮件到 {to_email}...")
        log.info(f"SMTP 服务器: {smtp_server}:{smtp_port}")
        log.info(f"发件人: {from_email}")
        ...
        try:
            ...
            log.info(f"邮件发送成功 [{report_type}] -> {to_email}")
            return True

        except smtplib.SMTPServerDisconnected:
            log.info("邮件发送失败：服务器意外断开连接，请检查网络或稍后重试")
            return False

    except smtplib.SMTPAuthenticationError as e:
        log.info("邮件发送失败：认证错误，请检查邮箱和密码/授权码")
        log.info(f"详细错误: {str(e)}")
        return False
    except smtplib.SMTPRecipientsRefused as e:
        log.info(f"邮件发送失败：收件人地址被拒绝 {e}")
        return False
    except smtplib.SMTPSenderRefused as e:
        log.info(f"邮件发送失败：发件人地址被拒绝 {e}")
        return False
    except smtplib.SMTPDataError as e:
        log.info(f"邮件发送失败：邮件数据错误 {e}")
        return False
    except smtplib.SMTPConnectError as e:
        log.info(f"邮件发送失败：无法连接到 SMTP 服务器 {smtp_server}:{smtp_port}")
        log.info(f"详细错误: {str(e)}")
        return False
    except Exception as e:
        log.info(f"邮件发送失败 [{report_type}]：{e}")
        import traceback
        traceback.print_exc()
        return False
...
def send_to_ntfy(
    ...
    *,
    ...
    log.info(f"{log_prefix}消息分为 {total_batches} 批次发送 [{report_type}]")

    # 反转批次顺序，使得在ntfy客户端显示时顺序正确
    # ntfy显示最新消息在上面，所以我们从最后一批开始推送
    reversed_batches = list(reversed(batches))

    log.info(f"{log_prefix}将按反向顺序推送（最后批次先推送），确保客户端显示顺序正确")
    ...
    for idx, batch_content in enumerate(reversed_batches, 1):
        ...
        if content_size > 4096:
            log.info(f"警告：{log_prefix}第 {actual_batch_num} 批次消息过大（{content_size} 字节），可能被拒绝")
        ...
        try:
            ...
            if response.status_code == 200:
                log.info(f"{log_prefix}第 {actual_batch_num}/{total_batches} 批次发送成功 [{report_type}]")
                success_count += 1
                ...
                if retry_response.status_code == 200:
                    log.info(f"{log_prefix}第 {actual_batch_num}/{total_batches} 批次重试成功 [{report_type}]")
                    success_count += 1
                else:
                    log.info(
            ...
            log.info(f"{log_prefix}第 {actual_batch_num}/{total_batches} 批次连接超时 [{report_type}]")
        except requests.exceptions.ReadTimeout:
            log.info(f"{log_prefix}第 {actual_batch_num}/{total_batches} 批次读取超时 [{report_type}]")
        except requests.exceptions.ConnectionError as e:
            log.info(f"{log_prefix}第 {actual_batch_num}/{total_batches} 批次连接错误 [{report_type}]：{e}")
        except Exception as e:
            log.info(f"{log_prefix}第 {actual_batch_num}/{total_batches} 批次发送异常 [{report_type}]：{e}")

    # 判断整体发送是否成功
    if success_count == total_batches:
        log.info(f"{log_prefix}所有 {total_batches} 批次发送完成 [{report_type}]")
    elif success_count > 0:
        log.info(f"{log_prefix}部分发送成功：{success_count}/{total_batches} 批次 [{report_type}]")
    else:
        log.info(f"{log_prefix}发送完全失败 [{report_type}]")
        return False
...
def send_to_bark(
    ...
    *,
    ...
    if not device_key:
        log.info(f"{log_prefix} URL 格式错误，无法提取 device_key: {bark_url}")
        return False
        ...
        try:
            ...
            if payload_template:
                ...
                try:
                    ...
                    log.info(f"{log_prefix} JSON 模板解析失败: {e}")
                    # 回退到默认格式
